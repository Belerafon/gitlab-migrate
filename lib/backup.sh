...
restore_backup_if_needed() {
  ...
  while [ $restore_attempt -le $max_attempts ]; do
    ...
    trap - ERR
    set +e
    dexec "set -o pipefail; umask 077; ( time gitlab-backup restore BACKUP=$ts force=yes ) 2>&1 | tee '${rlog}' | grep -E '^( \*[^ ]|Warning:|ERROR|FATAL|Starting Chef Client|Recipe:|Running handlers|Chef Client finished|gitlab Reconfigured|Restore task is done|real|user|sys)' | grep -vE 'ERROR: +(relation|sequence|table) .* does not exist|ERROR: +must be owner of extension' || true; exit \\${PIPESTATUS[0]}"
    cmd_rc=$?
    set -e
    trap error_trap ERR
    if [ $cmd_rc -eq 0 ]; then
